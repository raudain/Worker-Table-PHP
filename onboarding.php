...
class Onboarding {
	...
	protected function update() {
		...
		$cost = $this->calculateCost($workerProfession, $workerEndurance);
		
		$newWorker = new Worker();
		$newWorker->setRoom($roomNumber);
		...
		$myfile = fopen("C:\\Users\\Rudy1\\Documents\\" +
		      "GitHub\\Virtual-Beggar-Servlet\\Worker-Table-PHP\\" +
              "Worker Export.txt", "w")
              or die("Unable to open file!");
        foreach ($workerList as $worker) {
            fwrite($myfile, $worker . "\n");
        }
        fclose($myfile);
	}
	 ...
	 *
	 */
	private function calculateCost($profession,
			$endurance) {
